class Tictactoe{
	...
	userplay(move){
		...
		console.log(this.boardfield[x][y]);
		if(this.boardfield[x][y]!=0)
		{
	...
	checkline(player,line,boardtemp){
		...
		console.log(x3);
		var boola = boardtemp[x1][y1] == player &&
				boardtemp[x2][y2] == player &&
				boardtemp[x3][y3] == player;
	...
	nextPlay()
	{
		...
		var x = parseInt(pos.substring(0,1));
		var y = parseInt(pos.substring(1,2));

		this.boardfield[x][y]=1;
	...
	checkNextWin(possArray,user){
		for(var i = 0; i < possArray.length; i++)
		{
			...
			var x = parseInt(possArray[i].substring(0,1));
			var y = parseInt(possArray[i].substring(1,2));
	...
	checkNextNormal(possArray){
		...
		if(this.hard)
		{
			...
			{
				var x = parseInt(prefsmovs[i].substring(0,1));
				var y = parseInt(prefsmovs[i].substring(1,2));
				if(this.boardfield[x][y]==0)
					return prefsmovs[i];
			}
		}
		else
		{
			while(true)
			{
				...
				var x = pos.substring(0,1);
				var y = pos.substring(1,2);
				if(this.boardfield[x][y]==0)
					return pos;
			}
		}
